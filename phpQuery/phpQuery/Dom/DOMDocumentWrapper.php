...
class phpQuery_Dom_DOMDocumentWrapper
{
	...
	{
		if (isset($markup)) $this->load($markup, $contentType, $newDocumentID);
		...
		if ($this->contentType)
		{
			...
			switch ($contentType)
			{
				...
				default:
					...
					else  phpQuery::debug("Could not determine document type from content type '{$this->contentType}'");
			}
		}
		else
		{
	...
	{
		if (phpQuery::$debug) phpQuery::debug('Full markup load (HTML): ' . substr($markup, 0, 250));
		$this->loadMarkupReset();
		$this->isHTML = true;
		if (!isset($this->isDocumentFragment)) $this->isDocumentFragment = self::isDocumentFragmentHTML($markup);
		$charset = null;
		...
		if (!$charset) $charset = phpQuery::$defaultCharset;
		...
		if ($requestedCharset && $documentCharset && $requestedCharset !== $documentCharset)
		{
			...
			if (function_exists('mb_detect_encoding'))
			{
				...
				if (!$docEncoding) $docEncoding = $documentCharset; // ok trust the document
		...
		else
		{
			...
			if ($return) $this->root = $this->document;
		}
		if ($return && !$this->contentType) $this->contentType = 'text/html';
		return $return;
	}
	protected function loadMarkupXML($markup, $requestedCharset = null)
	{
		if (phpQuery::$debug) phpQuery::debug('Full markup load (XML): ' . substr($markup, 0, 250));
		$this->loadMarkupReset();
		$this->isXML = true;
		...
		if (!isset($this->isDocumentFragment)) $this->isDocumentFragment = $this->isXHTML ? self::isDocumentFragmentXHTML($markup) : self::isDocumentFragmentXML($markup);
		...
		if ($return)
		{
			if (!$this->contentType)
			{
				if ($this->isXHTML) $this->contentType = 'application/xhtml+xml';
				else  $this->contentType = 'text/xml';
			}
			return $return;
		}
		else
	...
	protected function contentTypeFromHTML($markup)
	{
		...
		if (!isset($matches[0])) return array(null, null);
		// get attr 'content'
	...
	public function import($source, $sourceCharset = null)
	{
		...
		$return = array();
		if ($source instanceof DOMNODE && !($source instanceof DOMNODELIST)) $source = array($source);
		//		if (is_array($source)) {
		...
		if (is_array($source) || $source instanceof DOMNODELIST)
		{
			...
			foreach ($source as $node) $return[] = $this->document->importNode($node, true);
		}
		else
		{
			// string markup
			$fake = $this->documentFragmentCreate($source, $sourceCharset);
			if ($fake === false) throw new Exception("Error loading documentFragment markup");
			else  return $this->import($fake->root->childNodes);
		}
		return $return;
	}
	/**
	...
	protected function documentFragmentCreate($source, $charset = null)
	{
		...
		if (is_array($source) || $source instanceof DOMNODELIST)
		{
			// dom nodes
			// load fake document
			if (!$this->documentFragmentLoadMarkup($fake, $charset)) return false;
			$nodes = $fake->import($source);
			foreach ($nodes as $node) $fake->root->appendChild($node);
		}
		else
		{
			// string markup
	...
	protected function documentFragmentToMarkup($fragment)
	{
		...
		if (phpQuery::$debug) phpQuery::debug('documentFragmentToMarkup: ' . substr($markup, 0, 150));
		return $markup;
	}
	/**
	...
	{
		if (isset($nodes) && count($nodes) == 1 && $nodes[0] instanceof DOMDOCUMENT) $nodes = null;
		if (isset($nodes))
		{
			$markup = '';
			if (!is_array($nodes) && !($nodes instanceof DOMNODELIST)) $nodes = array($nodes);
			...
			if ($this->isXML && !$innerMarkup)
			{
				...
				foreach ($nodes as $node) $markup .= $this->document->saveXML($node);
			}
			else
			{
				$loop = array();
				if ($innerMarkup)
					...
					{
						if ($node->childNodes)
							foreach ($node->childNodes as $child) $loop[] = $child;
							else  $loop[] = $node;
					}
				else  $loop = $nodes;
