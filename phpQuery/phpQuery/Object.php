...
//				$i = $i+2;
	...
	public function newInstance($newStack = null) {
		...
		$new = $class != 'phpQuery'
			...
			: new phpQuery_Object($this->getDocumentID());
		$new->previous = $this;
		if (is_null($newStack)) {
			...
			case 'reset':
				$this->elements = phpQuery::merge(
					$this->map(array($this, 'is'),
						"input[type=$class]", new phpQuery_Callback_Param()
					),
					$this->map(array($this, 'is'),
						"button[type=$class]", new phpQuery_Callback_Param()
					)
				);
			break;
//				$stack = array();
...
//				$this->elements = $stack;
			case 'input':
				$this->elements = $this->map(
					array($this, 'is'),
					'input', new phpQuery_Callback_Param()
				)->elements;
			break;
			case 'password':
			case 'checkbox':
			...
			case 'checked':
				$this->elements = $this->map(
					array($this, 'is'),
					"[$class]", new phpQuery_Callback_Param()
				)->elements;
			break;
			case 'enabled':
				$this->elements = $this->map(
...
//		$this->previous->XPath = $this->XPath;
	...
	public function remove($selector = null) {
		...
		foreach($loop as $node) {
			...
			$node->parentNode->removeChild($node);
			// Mutation event
			$event = new phpQuery_Dom_DOMEvent(array(
				'target' => $node,
				'type' => 'DOMNodeRemoved'
			));
...
//						);
	...
	public function index($subject) {
		$index = -1;
		$subject = $subject instanceof phpQuery_Object
			? $subject->elements[0]
			: $subject;
