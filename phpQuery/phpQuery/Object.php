 ...
 * @package phpQuery
 * @method phpQuery_Object clone() clone()
 * @method phpQuery_Object empty() empty()
 * @method phpQuery_Object next() next($selector = null)
 * @method phpQuery_Object prev() prev($selector = null)
 * @property Int $length
 */
class phpQuery_Object implements Iterator, Countable, ArrayAccess
{
	...
	/**
	 *
	 * @var phpQuery_Dom_DOMDocumentWrapper
	 */
	...
	public function serializeArray($submit = null)
	{
		...
		foreach ($source as $input)
		{
			...
			if (!$input->is('[name]')) continue;
			if ($input->is('[type=checkbox]') && !$input->is('[checked]')) continue;
			// jquery diff
			if ($submit && $input->is('[type=submit]'))
			{
				if ($submit instanceof DOMELEMENT && !$input->elements[0]->isSameNode($submit)) continue;
				else
					if (is_string($submit) && $input->attr('name') != $submit) continue;
			}
			$return[] = array(
	...
	protected function parseSelector($query)
	{
		...
		$_query = array();
		for ($i = 0; $i < $strlen; $i++) $_query[] = mb_substr($query, $i, 1);
		$query = $_query;
		...
		foreach ($queries as $k => $q)
		{
			if (isset($q[0]))
			{
				if (isset($q[0][0]) && $q[0][0] == ':') array_unshift($queries[$k], '*');
				if ($q[0] != '>') array_unshift($queries[$k], ' ');
			}
		}
		return $queries;
	}
	...
	public function get($index = null, $callback1 = null, $callback2 = null, $callback3 = null)
	{
		...
		{
			if (is_array($return))
				foreach ($return as $k => $v) $return[$k] = phpQuery::callbackRun($callback, array($v));
				else  $return = phpQuery::callbackRun($callback, array($return));
		}
		return $return;
	}
	/**
	...
	{
		if ($index) $return = $this->eq($index)->text();
		else
		{
			$return = array();
	...
	public function newInstance($newStack = null)
	{
		...
		$new = $class != 'phpQuery' ? new $class($this, $this->getDocumentID()) : new phpQuery_Object($this->getDocumentID());
		$new->previous = $this;
		if (is_null($newStack))
		{
			$new->elements = $this->elements;
			if ($this->elementsBackup) $this->elements = $this->elementsBackup;
		}
		else
	...
	protected function matchClasses($class, $node)
	{
		...
		if (mb_strpos($class, '.', 1))
		{
			...
			if ($classesCount > $nodeClassesCount) return false;
	...
	{
		if ($compare && !method_exists($this, $compare)) return false;
		$stack = array();
		if (!$this->elements) $this->debug('Stack empty, skipping...');
			...
			13)) as $k => $stackNode)
		{
			...
			if (!$nodes->length) $this->debug('Nothing found');
			$debug = array();
			foreach ($nodes as $node)
			{
				...
				if ($compare)
				{
					...
					if (call_user_func_array(array($this, $compare), array($selector, $node))) $matched = true;
				...
				if ($matched)
				{
					if (phpQuery::$debug) $debug[] = $this->whois($node);
					$stack[] = $node;
				}
			}
			if (phpQuery::$debug)
			...
			if ($detachAfter) $this->root->removeChild($detachAfter);
		}
		$this->elements = $stack;
	}
	/**
	...
	public function find($selectors, $context = null, $noHistory = false)
	{
		...
		if ($context)
		{
			if (!is_array($context) && $context instanceof DOMELEMENT) $this->elements = array($context);
			else
				if (is_array($context))
				{
					$this->elements = array();
					foreach ($context as $c)
						if ($c instanceof DOMELEMENT) $this->elements[] = $c;
				}
				else
					if ($context instanceof self) $this->elements = $context->elements;
		}
		...
		foreach ($queries as $selector)
		{
								...
								if ($s[0] == '~')
								{
									...
									if ($s[0] == '+')
									{
										...
										foreach ($subElements as $node)
										{
											...
											while ($test && !($test instanceof DOMELEMENT)) $test = $test->nextSibling;
											if ($test && $this->is($subSelector, $test)) $this->elements[] = $test;
										}
										if (!$this->length()) break;
										// PSEUDO CLASSES
									}
			...
			foreach ($this->elements as $node)
				if (!$this->elementsContainsNode($node, $stack)) $stack[] = $node;
		}
		$this->elements = $stack;
	...
	protected function pseudoClasses($class)
	{
		...
		switch ($class)
		{
			...
			case 'odd':
				...
				{
					if ($class == 'even' && ($i % 2) == 0) $stack[] = $node;
					else
						if ($class == 'odd' && $i % 2) $stack[] = $node;
				}
				$this->elements = $stack;
				break;
			case 'eq':
			...
			case 'lt':
				...
				break;
			case 'first':
				if (isset($this->elements[0])) $this->elements = array($this->elements[0]);
				break;
			case 'last':
				if ($this->elements) $this->elements = array($this->elements[count($this->elements) - 1]);
				break;
				/*case 'parent':
				$stack = array();
				foreach($this->elements as $node) {
			...
			case 'contains':
				...
				foreach ($this->elements as $node)
				{
					if (mb_stripos($node->textContent, $text) === false) continue;
					$stack[] = $node;
				}
				$this->elements = $stack;
				break;
			...
			case 'has':
				...
				{
					if ($this->find($selector, $el, true)->length) $stack[] = $el;
				}
				$this->elements = $stack;
				break;
			case 'submit':
			case 'reset':
				$this->elements = phpQuery::merge($this->map(array($this, 'is'), "input[type=$class]", new phpQuery_Callback_Param()), $this->map(array($this, 'is'), "button[type=$class]", new phpQuery_Callback_Param()));
				break;
				//				$stack = array();
				//				foreach($this->elements as $node)
				...
				//				$this->elements = $stack;
			case 'input':
				$this->elements = $this->map(array($this, 'is'), 'input', new phpQuery_Callback_Param())->elements;
				break;
			case 'password':
			case 'checkbox':
			case 'radio':
			...
			case 'image':
			case 'file':
				$this->elements = $this->map(array($this, 'is'), "input[type=$class]", new phpQuery_Callback_Param())->elements;
				break;
			case 'parent':
			...
			case 'selected':
			case 'checked':
				$this->elements = $this->map(array($this, 'is'), "[$class]", new phpQuery_Callback_Param())->elements;
				break;
			case 'enabled':
				$this->elements = $this->map(create_function('$node', '
			...
			case 'nth-child':
				...
				if ($param{0} == 'n') $param = '1' . $param;
				// :nth-child(index/even/odd/equation)
				if ($param == 'even' || $param == 'odd') $mapped = $this->map(create_function('$node, $param', '$index = pq($node)->prevAll()->size()+1;
							if ($param == "even" && ($index%2) == 0)
								return $node;
							else if ($param == "odd" && $index%2 == 1)
								return $node;
							else
								return null;'), new phpQuery_Callback_Param(), $param);
				else
...
//									? $node
//									: null;
							'), new phpQuery_Callback_Param(), $param);
					else // index
	...
	public function is($selector, $nodes = null)
	{
		...
		if ($nodes && is_array($nodes))
		{
			...
			if ($nodes) $this->elements = array($nodes);
		$this->filter($selector, true);
		$stack = $this->elements;
		$this->elements = $oldStack;
		if ($nodes) return $stack ? $stack : null;
	...
	public function filterCallback($callback, $_skipHistory = false)
	{
		...
		foreach ($this->elements as $index => $node)
		{
			...
			if (is_null($result) || (!is_null($result) && $result)) $newStack[] = $node;
		}
	...
	{
		if ($selectors instanceof phpQuery_Callback or $selectors instanceof Closure) return $this->filterCallback($selectors, $_skipHistory);
		if (!$_skipHistory) $this->elementsBackup = $this->elements;
		$notSimpleSelector = array(
			...
			'+',
			'/');
		if (!is_array($selectors)) $selectors = $this->parseSelector($selectors);
		if (!$_skipHistory) $this->debug(array("Filtering:", $selectors));
		$finalStack = array();
		foreach ($selectors as $selector)
		{
			...
			if (in_array($selector[0], $notSimpleSelector)) $selector = array_slice($selector, 1);
			...
			foreach ($this->stack() as $node)
			{
				...
				foreach ($selector as $s)
				{
					if (!($node instanceof DOMELEMENT))
					{
						...
						if ($s[0] == '[')
						{
							...
							if (mb_strpos($attr, '='))
							{
								...
								if ($attr == 'nodeType' && $node->nodeType != $val) $break = true;
							}
						}
						else  $break = true;
					}
					else
					{
						...
						if ($s[0] == '#')
						{
							if ($node->getAttribute('id') != substr($s, 1)) $break = true;
							// CLASSES
						}
						else
							if ($s[0] == '.')
							{
								if (!$this->matchClasses($s, $node)) $break = true;
								// ATTRS
							}
							else
								if ($s[0] == '[')
								{
									...
									if (mb_strpos($attr, '='))
									{
										...
										if ($attr == 'nodeType')
										{
											if ($val != $node->nodeType) $break = true;
										}
										else
											if ($this->isRegexp($attr))
											{
												...
												if (!$isMatch) $break = true;
											}
											else
												if ($node->getAttribute($attr) != $val) $break = true;
									}
									else
										if (!$node->hasAttribute($attr)) $break = true;
									// PSEUDO CLASSES
								}
								else
									if ($s[0] == ':')
									{
										...
										if (trim($s))
										{
											if ($s != '*')
											{
												...
												{
													if ($node->tagName != $s) $break = true;
												}
												else
													if ($s == 'html' && !$this->isRoot($node)) $break = true;
											}
	...
	{
		foreach ($this->elements as $node) phpQuery_Events::trigger($this->getDocumentID(), $type, $data, $node);
		return $this;
	}
	/**
	...
	public function bind($type, $data, $callback = null)
	{
		...
		foreach ($this->elements as $node) phpQuery_Events::add($this->getDocumentID(), $node, $type, $data, $callback);
		return $this;
	}
	/**
	 * Enter description here...
	...
	{
		foreach ($this->elements as $node) phpQuery_Events::remove($this->getDocumentID(), $node, $type, $callback);
		return $this;
	}
	/**
	 * Enter description here...
	...
	{
		if ($callback) return $this->bind('change', $callback);
	...
	{
		if ($callback) return $this->bind('submit', $callback);
	...
	{
		if ($callback) return $this->bind('click', $callback);
	...
	public function wrapAllOld($wrapper)
	{
		...
		if (!$wrapper->length() || !$this->length()) return $this;
		$wrapper->insertBefore($this->elements[0]);
		$deepest = $wrapper->elements[0];
		while ($deepest->firstChild && $deepest->firstChild instanceof DOMELEMENT) $deepest = $deepest->firstChild;
	...
	{
		foreach ($this->stack() as $node) phpQuery::pq($node, $this->getDocumentID())->wrapAll($wrapper);
		return $this;
	}
	/**
	 * Enter description here...
	...
	{
		foreach ($this->stack() as $node) phpQuery::pq($node, $this->getDocumentID())->wrapAllPHP($codeBefore, $codeAfter);
		return $this;
	}
	/**
	 * Enter description here...
	...
	{
		foreach ($this->stack() as $node) phpQuery::pq($node, $this->getDocumentID())->contents()->wrapAll($wrapper);
		return $this;
	}
	/**
	 * Enter description here...
	...
	{
		foreach ($this->stack(1) as $node) phpQuery::pq($node, $this->getDocumentID())->contents()->wrapAllPHP($codeBefore, $codeAfter);
		return $this;
	}
	/**
	 * Enter description here...
	...
	public function contentsUnwrap()
	{
		foreach ($this->stack(1) as $node)
		{
			...
			foreach ($node->childNodes as $chNode) $childNodes[] = $chNode;
	...
	public function eq($num)
	{
		...
		$this->elementsBackup = $this->elements;
		$this->elements = array();
		if (isset($oldStack[$num])) $this->elements[] = $oldStack[$num];
		return $this->newInstance();
	}
	/**
	 * Enter description here...
	...
	{
		foreach (phpQuery::pq($selector, $this->getDocumentID()) as $node) phpQuery::pq($node, $this->getDocumentID())->after($this->_clone())->remove();
		return $this;
	}
	/**
	 * Enter description here...
	...
	public function remove($selector = null)
	{
		...
		{
			if (!$node->parentNode) continue;
			if (isset($node->tagName)) $this->debug("Removing '{$node->tagName}'");
			$node->parentNode->removeChild($node);
			// Mutation event
			$event = new phpQuery_Dom_DOMEvent(array('target' => $node, 'type' => 'DOMNodeRemoved'));
			phpQuery_Events::trigger($this->getDocumentID(), $event->type, array($event), $node);
		}
		return $this;
	}
	protected function markupEvents($newMarkup, $oldMarkup, $node)
	{
		...
		{
			$event = new phpQuery_Dom_DOMEvent(array('target' => $node, 'type' => 'change'));
			phpQuery_Events::trigger($this->getDocumentID(), $event->type, array($event), $node);
		}
	}
	/**
	...
	{
		$args = func_get_args();
		if ($this->documentWrapper->isXML) return call_user_func_array(array($this, 'xml'), $args);
		else  return call_user_func_array(array($this, 'html'), $args);
	}
	/**
	 * jQuey difference
	 *
	...
	{
		$args = func_get_args();
		if ($this->documentWrapper->isXML) return call_user_func_array(array($this, 'xmlOuter'), $args);
		else  return call_user_func_array(array($this, 'htmlOuter'), $args);
	}
	/**
	 * Enter description here...
	 *
	...
	public function html($html = null, $callback1 = null, $callback2 = null, $callback3 = null)
	{
		if (isset($html))
		{
			...
			foreach ($this->stack(1) as $alreadyAdded => $node)
			{
				...
				if (($this->isXHTML() || $this->isHTML()) && $node->tagName == 'textarea') $oldHtml = pq($node, $this->getDocumentID())->markup();
				foreach ($nodes as $newNode)
				{
					$node->appendChild($alreadyAdded ? $newNode->cloneNode(true) : $newNode);
				}
				// for now, limit events for textarea
				if (($this->isXHTML() || $this->isHTML()) && $node->tagName == 'textarea') $this->markupEvents($html, $oldHtml, $node);
			}
			return $this;
		}
		else
	...
	public function children($selector = null)
	{
		...
		foreach ($this->stack(1) as $node)
		{
			...
			{
				if ($newNode->nodeType != 1) continue;
				if ($selector && !$this->is($selector, $newNode)) continue;
				if ($this->elementsContainsNode($newNode, $stack)) continue;
				$stack[] = $newNode;
			}
		}
		$this->elementsBackup = $this->elements;
		...
		switch (gettype($target))
		{
			...
			case 'object':
				...
				if ($target instanceof self)
				{
					...
					{
						$insertTo = $this->elements;
						if ($target->documentFragment && $target->stackIsRoot()) // get all body children
							//							$loop = $target->find('body > *')->elements;
 							$loop = $target->root->childNodes;
						else  $loop = $target->elements;
						// import nodes if needed
				...
				elseif ($target instanceof DOMNODE)
				{
					...
					{
						// import node if needed
						if (!$target->ownerDocument->isSameNode($this->document)) $target = $this->document->importNode($target, true);
						$insertTo = $this->elements;
						$insertFrom[] = $target;
					}
				}
		...
		foreach ($insertTo as $insertNumber => $toNode)
		{
			...
			foreach ($insertFrom as $fromNode)
			{
				...
				switch ($type)
				{
					...
					case 'insertBefore':
					case 'before':
						if (!$toNode->parentNode) throw new Exception("No parentNode, can't do {$type}()");
						else  $toNode->parentNode->insertBefore($insert, $toNode);
						break;
					case 'insertAfter':
					case 'after':
						if (!$toNode->parentNode) throw new Exception("No parentNode, can't do {$type}()");
						else  $toNode->parentNode->insertBefore($insert, $nextSibling);
						break;
				}
				// Mutation event
				$event = new phpQuery_Dom_DOMEvent(array('target' => $insert, 'type' => 'DOMNodeInserted'));
				phpQuery_Events::trigger($this->getDocumentID(), $event->type, array($event), $insert);
			}
		}
		return $this;
	}
	...
	{
		$index = -1;
		$subject = $subject instanceof phpQuery_Object ? $subject->elements[0] : $subject;
		foreach ($this->newInstance() as $k => $node)
		{
			if ($node->isSameNode($subject)) $index = $k;
		}
		return $index;
	}
	/**
	...
	{
		if (isset($text)) return $this->html(htmlspecialchars($text));
		$args = func_get_args();
		$args = array_slice($args, 1);
		$return = '';
		foreach ($this->elements as $node)
		{
			...
			if (count($this->elements) > 1 && $text) $text .= "\n";
			foreach ($args as $callback)
			{
	...
	public function __call($method, $args)
	{
		...
		if (isset(phpQuery::$extendMethods[$method]))
		{
			...
			if (isset(phpQuery::$pluginsMethods[$method]))
			{
				...
				else  throw new Exception("Method '{$method}' doesnt exist");
	}
	/**
	 * Safe rename of next().
	 *
	...
	protected function getElementSiblings($direction, $selector = null, $limitToOne = false)
	{
		...
		foreach ($this->stack() as $node)
		{
			...
			{
				$test = $test->{$direction};
				if (!$test instanceof DOMELEMENT) continue;
				$stack[] = $test;
				if ($limitToOne) break;
			}
		}
	...
	{
		if (is_string($selector)) phpQuery::debug(array('not', $selector));
		else  phpQuery::debug('not');
		$stack = array();
		if ($selector instanceof self || $selector instanceof DOMNODE)
		{
			foreach ($this->stack() as $node)
			{
				if ($selector instanceof self)
				{
					...
					{
						if ($notNode->isSameNode($node)) $matchFound = true;
					}
					if (!$matchFound) $stack[] = $node;
				}
				else
					if ($selector instanceof DOMNODE)
					{
						if (!$selector->isSameNode($node)) $stack[] = $node;
					}
					else
					{
						if (!$this->is($selector)) $stack[] = $node;
					}
			}
		}
		else
		{
			...
			foreach ($orgStack as $node)
				if (!$this->elementsContainsNode($node, $matched)) $stack[] = $node;
		}
		return $this->newInstance($stack);
	}
	/**
	...
	protected function merge()
	{
		foreach (func_get_args() as $nodes)
			foreach ($nodes as $newNode)
				if (!$this->elementsContainsNode($newNode)) $this->elements[] = $newNode;
	}
	/**
	 * @access private
	...
	protected function elementsContainsNode($nodeToCheck, $elementsStack = null)
	{
		...
		foreach ($loop as $node)
		{
			if ($node->isSameNode($nodeToCheck)) return true;
		}
		return false;
	}
	/**
	...
	public function parent($selector = null)
	{
		$stack = array();
		foreach ($this->elements as $node)
			if ($node->parentNode && !$this->elementsContainsNode($node->parentNode, $stack)) $stack[] = $node->parentNode;
		$this->elementsBackup = $this->elements;
		$this->elements = $stack;
	...
	{
		$stack = array();
		if (!$this->elements) $this->debug('parents() - stack empty');
		foreach ($this->elements as $node)
		{
			$test = $node;
	...
	{
		if (!isset($nodeTypes)) return $this->elements;
		if (!is_array($nodeTypes)) $nodeTypes = array($nodeTypes);
		$return = array();
		foreach ($this->elements as $node)
		{
			if (in_array($node->nodeType, $nodeTypes)) $return[] = $node;
		}
		return $return;
	}
	...
	protected function attrEvents($attr, $oldAttr, $oldValue, $node)
	{
		...
		if (!$this->isXHTML() && !$this->isHTML()) return;
		$event = null;
		// identify
		...
		if ($isInputValue && $attr == 'value' && $oldValue != $node->getAttribute($attr))
		{
				...
				|| (!$node->hasAttribute($attr) && $oldAttr)))
			{
				...
				if ($isOption && $node->parentNode && $attr == 'selected' && ( // select
					...
					|| (!$node->hasAttribute($attr) && $oldAttr)))
				{
					$event = new phpQuery_Dom_DOMEvent(array('target' => $node->parentNode, 'type' => 'change'));
				}
		if ($event)
		{
			phpQuery_Events::trigger($this->getDocumentID(), $event->type, array($event), $node);
		...
		foreach ($this->stack(1) as $node)
		{
			if (!is_null($value))
			{
				...
				if ($attr == '*')
				{
					// jQuery difference
					$return = array();
					foreach ($node->attributes as $n => $v) $return[$n] = $v->value;
					return $return;
				}
				else  return $node->hasAttribute($attr) ? $node->getAttribute($attr) : null;
		}
	...
	{
		$return = array();
		foreach ($node->attributes as $n => $o) $return[] = $n;
		return $return;
	}
	/**
	 * Enter description here...
	...
	public function val($val = null)
	{
		if (!isset($val))
		{
			if ($this->eq(0)->is('select'))
			{
				...
				if ($selected->is('[value]')) return $selected->attr('value');
				else  return $selected->text();
			}
			else
				if ($this->eq(0)->is('textarea')) return $this->eq(0)->markup();
				else  return $this->eq(0)->attr('value');
		}
		else
		{
			$_val = null;
			foreach ($this->stack(1) as $node)
			{
				...
				if (is_array($val) && in_array($node->attr('type'), array('checkbox', 'radio')))
				{
					...
					if ($isChecked) $node->attr('checked', 'checked');
					...
					if ($node->get(0)->tagName == 'select')
					{
						...
						{
							$_val = array();
							if (!is_array($val)) $_val = array((string )$val);
							else
								foreach ($val as $v) $_val[] = $v;
						}
						...
						if ($node->get(0)->tagName == 'textarea') $node->markup($val);
						else  $node->attr('value', $val);
			}
		}
		return $this;
	}
	...
	{
		if ($this->previous) $this->elements = array_merge($this->elements, $this->previous->elements);
		return $this;
	}
	/**
	 * Enter description here...
	...
	public function addClass($className)
	{
		...
		foreach ($this->stack(1) as $node)
		{
			if (!$this->is(".$className", $node)) $node->setAttribute('class', trim($node->getAttribute('class') . ' ' . $className));
		}
		return $this;
	}
	/**
	...
	public function hasClass($className)
	{
		foreach ($this->stack(1) as $node)
		{
			if ($this->is(".$className", $node)) return true;
		}
		return false;
	}
	/**
	...
	public function removeClass($className)
	{
		foreach ($this->stack(1) as $node)
		{
			...
			if (in_array($className, $classes))
			{
				...
				if ($classes) $node->setAttribute('class', implode(' ', $classes));
	...
	public function toggleClass($className)
	{
		foreach ($this->stack(1) as $node)
		{
			if ($this->is($node, '.' . $className)) $this->removeClass($className);
	...
	public function each($callback, $param1 = null, $param2 = null, $param3 = null)
	{
		...
		foreach ($this->elements as $v) phpQuery::callbackRun($callback, array($v), $paramStructure);
		return $this;
	}
	/**
	...
	public function map($callback, $param1 = null, $param2 = null, $param3 = null)
	{
		...
		return $this->newInstance(call_user_func_array(array('phpQuery', 'map'), $params) //			phpQuery::map($this->elements, $callback)
			);
	}
	/**
	 * Enter description here...
	...
	public function data($key, $value = null)
	{
		...
		{
			foreach ($this as $node) phpQuery::data($node, $key, $value, $this->getDocumentID());
			return $this;
		}
	}
	/**
	...
	{
		foreach ($this as $node) phpQuery::removeData($node, $key, $this->getDocumentID());
		return $this;
	}
		...
		foreach ($loop as $node)
		{
			...
			while (!($node instanceof DOMDOCUMENT))
			{
				...
				while ($sibling->previousSibling)
				{
					...
					if ($isElement && $sibling->tagName == $node->tagName) $i++;
				}
	...
	public function whois($oneNode = null)
	{
		...
		foreach ($loop as $node)
		{
			if (isset($node->tagName))
			{
				...
				{
					if (trim($node->textContent)) $return[] = 'Text:' . substr(str_replace("\n", ' ', $node->textContent), 0, 15);
				}
				else
				{
	...
	public function dumpTree($html = true, $title = true)
	{
		...
		$debug = phpQuery::$debug;
		phpQuery::$debug = false;
		foreach ($this->stack() as $node) $output .= $this->__dumpTree($node);
		phpQuery::$debug = $debug;
	...
	private function __dumpTree($node, $intend = 0)
	{
		...
		if (isset($node->childNodes))
			foreach ($node->childNodes as $chNode) $return .= $this->__dumpTree($chNode, $intend + 1);
		return $return;
	}
	/**
